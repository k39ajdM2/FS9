...
nmdsplot <- ggplot(data=metanmds, aes(x=MDS1, y=MDS2, color=Treatment)) + geom_point() + 
  ...
  scale_color_manual(values = c(INFinject='#00BA38',
                                INFnm='#F8766D',
                                INFfeed='#619CFF')) +
  labs(caption = 'Ordination stress = 0.182')
...
ggsave("FS9_Q2_NMDS_Day.tiff", plot=nmdsplot_day, width = 10, height = 6, dpi = 500, units =c("in"))
...
ggsave("FS9_Q2_NMDS_Treatment.tiff", plot=nmdsplot_treatment, width = 10, height = 6, dpi = 500, units =c("in"))
...
ggsave("FS9_Q2_NMDS_DayAndTreatment_AllSamples.tiff", plot=nmdsplot_treatment2, width = 10, height = 6, dpi = 500, units =c("in"))


#Visualizing NMDS with only centroids plotted - this didn't work. Missing D11 and D14
...
#Restructure level order for 'metanmds' and 'df_ell'
unique(meta_with_NMDS$Day) #"D11" "D7"  "D14"
unique(df_ell$Day) #"D11" "D7"  "D14"
meta_with_NMDS$Day = factor(meta_with_NMDS$Day, levels = c("D7", "D11", "D14"))
df_ell$Day = factor(df_ell$Day, levels = c("D7", "D11", "D14"))
levels(df_ell$Day) #"D7"  "D11" "D14"
levels(meta_with_NMDS$Day) #"D7"  "D11" "D14"
...
#All days and treatments faceted by day (gridlines), no lines (only ellipse)
...
nmdsplot_treatment2
#Save 'nmdsplot_treatment2' as a .tiff for publication, 500dpi
ggsave("FS9_Q2_NMDS_DayAndTreatment_AllSamples_Centroid.tiff", plot=nmdsplot_treatment2, width = 10, height = 6, dpi = 500, units =c("in"))
...
ggsave(alphadiv,
       filename = './results/alpha_diversity_INFnm.feed.inject.jpeg',
       width = 300,
       height = 120,
       device = 'jpeg',
       dpi = 300,
