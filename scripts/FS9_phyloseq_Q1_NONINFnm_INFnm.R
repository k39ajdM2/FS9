#########################################
#FS9 16S phyloseq - Noninfected (NONINFnm) vs infected (INFnm), all days or only days 7, 11, 14
#Kathy Mou
...
meta <- read.csv("./data/FS9_metadata_NONINFvsINF_alldays.csv", row.names = 1)
...
#Read files for metadata and OTU table
meta <- read.csv("./data/FS9_metadata_NONINFnm_vs_INFnm_alldays.csv", row.names = 1)
otu <- read.csv("./data/FS9.OTUtable.doubleton.csv", row.names=1)
dim(otu) #1405 168
head(otu[,165:168])
